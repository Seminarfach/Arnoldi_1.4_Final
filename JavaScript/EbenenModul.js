import { schulhofConfig, Flur000Config, Flur000DanachConfig, InformatikRaumConfig} from './Ebenen/Ebene1.js';
import { EnglischRaumConfig, Flur100Config, Flur100DanachConfig } from './Ebenen/Ebene2.js';
import { GeschichtsRaumConfig, Flur200Config, Flur200DanachConfig } from './Ebenen/Ebene3.js';
import { MatheRaumConfig, Flur300Config, Flur300DanachConfig } from './Ebenen/Ebene4.js';
import { AulaConfig, Flur400Config, } from './Ebenen/Ebene5.js';
...
class LevelManager {
    ...
    if(this.shouldAdjustBackground && this.NewPlayerPosition) {
        ...
        if(level.backgroundAfter) {
            backgroundAfter.position.x = canvasCenterX - newanchor.x;
            backgroundAfter.position.y = canvasCenterY - newanchor.y;
        ...
        if(level.backgroundAfter) {
            backgroundAfter.position.x = canvasCenterX - anchor.x;
            backgroundAfter.position.y = canvasCenterY - anchor.y;
        ...
        if(level.backgroundAfter) {
            backgroundAfter.position.x = canvasCenterX - roomAnchor.x;
            backgroundAfter.position.y = canvasCenterY - roomAnchor.y;
            //console.log("Vordergrundposition:", foreground.position)
        ...
        for (let i = 0; i < zones.length; i++) {
            ...
            if (this.rectangularCollision({
                ...
                if (canMove) {
                    ...
                    if(this.drawbackgroundAfter && level.backgroundAfter ) {
                        backgroundAfter.position.x += offset.x
                        backgroundAfter.position.y += offset.y
    ...
    handleMovement() {
        ...
        } else if (keys.ArrowLeft.pressed || movingtouchLeft) {
        ...
        } else if (keys.ArrowDown.pressed || movingtouchDown) {
        ...
        } else if (keys.ArrowRight.pressed || movingtouchRight) {
                ...
                onComplete: () => {
                    gsap.to('#overlappingDiv', {
                        ...
                        onComplete: () => {
                            ...
                            // else if (this.config[this.currentLevel].flur) {

                            //     console.error("Aus Raum raus")
